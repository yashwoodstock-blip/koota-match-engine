...
from app.api.schemas import (
    ...
    CompatibilityCodesListResponse,
)
...
@router.get("/profiles/{profile_id}/compatibility-codes", response_model=CompatibilityCodesListResponse)
async def get_my_compatibility_codes(
    profile_id: str,
    db: AsyncSession = Depends(get_db),
    current_profile: Profile = Depends(get_current_authenticated_profile),
):
    """Retrieve all compatibility codes generated by the caller and their redemption results."""
    verify_profile_ownership(profile_id, current_profile)

    stmt = select(CompatibilityCode).where(
        CompatibilityCode.creator_profile_id == profile_id
    ).order_by(CompatibilityCode.created_at.desc())
    res = await db.execute(stmt)
    codes = res.scalars().all()

    items: List[CompatibilityCodeItemDTO] = []
    for c in codes:
        match_summary: Optional[CandidateMatchSummary] = None
        if c.is_used and c.used_by_profile_id:
            # Query match result
            m_stmt = select(MatchResult).where(
                MatchResult.profile_a_id == profile_id,
                MatchResult.profile_b_id == c.used_by_profile_id,
            ).order_by(MatchResult.created_at.desc())
            m_res = await db.execute(m_stmt)
            m = m_res.scalars().first()

            # Query candidate profile name
            cand_p_stmt = select(Profile).where(Profile.id == c.used_by_profile_id)
            cand_p_res = await db.execute(cand_p_stmt)
            cand_p = cand_p_res.scalar_one_or_none()

            if m:
                match_summary = CandidateMatchSummary(
                    candidate_id=c.used_by_profile_id,
                    candidate_name=cand_p.name if cand_p else "Prospective Match",
                    is_viable=m.is_viable,
                    tier=m.tier,
                    overall_score=m.overall_score,
                    alignment_points=m.alignment_points or [],
                    friction_points=m.friction_points or [],
                    disagreement_count=len(m.disagreement_flags or []),
        ...
        items.append(
            CompatibilityCodeItemDTO(
                ...
                is_used=c.is_used,
                used_by_profile_id=c.used_by_profile_id,
                used_at=c.used_at,
                match_result=match_summary,
            )
        )

    return CompatibilityCodesListResponse(
        profile_id=profile_id,
        codes=items,
    )

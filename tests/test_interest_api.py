...
async def test_interest_api_mutual_flow_and_staged_disclosure():
    ...
    with TestClient(app) as client:
        ...
        s_resp1 = client.get(f"/interest/{p1}/status", headers={"X-Test-Profile-Id": p1})
        ...
        s_resp2 = client.get(f"/interest/{p2}/status", headers={"X-Test-Profile-Id": p2})
        ...
        w_resp2 = client.get(f"/profiles/{p2}/weekly-matches", headers={"X-Test-Profile-Id": p2})
        ...
        s_resp1_after = client.get(f"/interest/{p1}/status", headers={"X-Test-Profile-Id": p1})
        assert s_resp1_after.json()["statuses"][0]["status"] == "mutual"
        assert s_resp1_after.json()["statuses"][0]["is_mutual"] is True

        s_resp2_after = client.get(f"/interest/{p2}/status", headers={"X-Test-Profile-Id": p2})
        ...
        w_resp1_after = client.get(f"/profiles/{p1}/weekly-matches", headers={"X-Test-Profile-Id": p1})
        ...
        w_resp2_after = client.get(f"/profiles/{p2}/weekly-matches", headers={"X-Test-Profile-Id": p2})
